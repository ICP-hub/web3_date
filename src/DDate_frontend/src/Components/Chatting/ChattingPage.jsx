...
//             <ChattingPageformessage profile={selectedProfile} onBack={handleBackClick} state={false} />
//           </div>
//         )}
//       </div>
//     </>
  ...
  useEffect(() => {
    const fetchChatHistory = async () => {
      ...
      try {
        ...
        const response = await fetch('https://ddate.kaifoundry.com/api/v1/login/user');  // Adjust the endpoint according to your API
  ...
  useEffect(() => {
    const fetchChatHistory = async () => {
      ...
      try {
        ...
        const response = await fetch('https://ddate.kaifoundry.com/api/v1/chat/history');  // Adjust the endpoint according to your API
        console.log("response", response);
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
    ...
    if (selectedProfile) {
      const fetchUserMessages = async () => {
        ...
        try {
          ...
          const response = await fetch(`https://ddate.kaifoundry.com/api/v1/chat/?principal&chat_id`); // Adjust the endpoint according to your API
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-12">
                ...
                {loading ? (
                  ...
                  chatHistory.map((pro, index) => (
                    <div key={pro.id}>
                      ...
                      {index !== chatHistory.length - 1 && <hr className="border-gray-300" />}
                    </div>
                  ))
                )}
              </div>
        ...
        {selectedProfile && (
          <div className="col-span-12 md:col-span-6 flex h-screen">
            <ChattingPageformessage profile={selectedProfile} onBack={handleBackClick} messages={messages} />
          </div>
        )}
      </div>
    </>
