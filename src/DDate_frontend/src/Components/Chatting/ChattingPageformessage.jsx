...
//     if (msg.fromPrincipal === "user1") return true;
//     else return false;
//   };
...
//                     key={index}
//                     className={`flex ${
//                       isMessageSent(msg) ? "justify-end" : ""
//                     } mb-2`}
//                   >
//                     <div
//                       className={`rounded-xl py-2 px-3 ${
//                         isMessageSent(msg) ? "bg-[#E2F7CB]" : "bg-[#F2F2F2]"
//                       }`}
//                     >
//                       <p className="text-sm mt-1">{msg.message}</p>
//                       <p className="text-xs opacity-75 pl-6 flex justify-end">
//                         {" "}
//                         {msg.dateTime}
//                       </p>
//                     </div>
...
const ChattingPageformessage = ({ messages, status, profile }) => {
  ...
  // const sortedMessages = [...receivedMessages, ...sentMessages].sort((a, b) => new Date(a.dateTime) - new Date(b.dateTime));
  const isMessageSent = (msg) => {
    console.log("isent", msg, profile.chat_id.split("-")[1])
    if (msg.to_user_id === profile.chat_id.split("-")[1]) return false;
    else return true;
  };
  const sortedMessages = [...messages, ...receivedMessages, ...sentMessages].sort(
    (a, b) => new Date(a.dateTime) - new Date(b.dateTime)
  );
        ...
        true ? (
          <div
            className="
                        border flex flex-col w-full float-right "
          >
            <div className="py-2 bg-[#DBEBF0] flex flex-row justify-between items-center px-4">
              <div className="flex gap-4 items-center">
                {/* <div className="flex items-center">
               <img
                 src={backarrow}
              ...
              <div className="flex">
               ...
               >
                ...
                <div className="flex space-x-2 cursor-pointer" onClick={() => navigate('/Swipe')}>
            ...
            <div className="relative flex justify-center items-center w-full h-8 bg-[#DBEBF0] px-4">
              <p className="border-t border-[#7A8588] w-full"></p>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="14"
                height="14"
              ...
              <div className="py-2 px-3" style={{ height: "84vh" }}>
                <div>
                  {sortedMessages.map((msg, index) => (
                    <div
                      key={index}
                      className={`flex ${isMessageSent(msg) ? "justify-end" : ""
                        } mb-2`}
                    >
                      <div
                        className={`rounded-xl py-2 px-3 ${isMessageSent(msg) ? "bg-[#ECFCFF]" : "bg-[#F1E2E9]"
                          }`}
                      >
                        <p className="text-sm mt-1">{msg.message}</p>
                        <p className="text-xs opacity-75 pl-6 flex justify-end">
            ...
            <div className="flex flex-row items-center h-16 rounded-xl w-full px-4">
              <div className="flex justify-center items-center w-full border rounded-full bg-[#F0F0F0] px-6 py-4 space-x-2 mb-6">
                <div>
                  <button className="flex items-center justify-center text-gray-400 hover:text-gray-600">
                    {/* <svg
                    className="w-5 h-5"
                ...
                <div className="ml-4">
                  <button
                    className="flex items-center justify-center px-4 py-1 flex-shrink-0"
                    onClick={sendMessage}
                  >
