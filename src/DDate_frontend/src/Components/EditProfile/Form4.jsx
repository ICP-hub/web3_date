...
const Form4 = () => {
  ...
  useEffect(() => {
    setValue("selectedPets", selectedPets);
  }, [])
          ...
          {["Dogs", "Cats", "Birds", "Others"].map((pet) => (
            <label
              ...
              className={`inline-block px-3 py-2 rounded-full text-sm focus:outline-none transition duration-300
                ${selectedPets === pet ? 'bg-yellow-500 text-black' : 'bg-transparent hover:bg-yellow-500 hover:text-black text-black border border-black'}`}
            >
              <input
                type="radio"
                value={pet}
